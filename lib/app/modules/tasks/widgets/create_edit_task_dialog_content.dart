...
import 'package:flutter/services.dart'; // <-- Импорт для Shortcuts/Actions
import 'package:get/get.dart';
import 'package:intl/intl.dart';
import '../controllers/tasks_controller.dart';

// Определяем Intent
class SubmitTaskIntent extends Intent {}
...
class CreateEditTaskDialogContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Определяем Action
    final submitTaskAction = CallbackAction<SubmitTaskIntent>(
      onInvoke: (intent) => controller.saveTaskFromDialog(),
    );

    // Определяем Shortcuts
    final Map<ShortcutActivator, Intent> shortcuts = {
      LogicalKeySet(LogicalKeyboardKey.enter): SubmitTaskIntent(),
      // Можно добавить Shift+Enter если нужно будет, но для форм обычно Enter
    };
      ...
      child: Obx(
        () => SingleChildScrollView(
          ...
          child: Shortcuts(
            shortcuts: shortcuts,
            child: Actions(
              actions: {SubmitTaskIntent: submitTaskAction},
              child: Form(
                key:
                    controller
                        .dialogFormKey, // Используем controller из параметра
                child: Column(
